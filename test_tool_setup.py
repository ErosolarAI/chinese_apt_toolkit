...
def test_tool_links():
    ...
    for campaign_path in test_campaigns:
        print(f"\n[*] Testing campaign: {campaign_path}")
        
        tools_dir = Path(campaign_path) / "tools"
...
def test_campaign_specific_tools():
    ...
    test_campaigns = {
        "campaigns/3d_printing_campaign": ["3d_model_finder.py"],
        "campaigns/accounting_firm_campaign": ["financial_record_finder.py"],
        "campaigns/aerospace_company_campaign": ["aerospace_design_finder.py"]
    }
    
    all_passed = True
    
    for campaign_path, expected_tools in test_campaigns.items():
        print(f"\n[*] Testing campaign: {campaign_path}")
